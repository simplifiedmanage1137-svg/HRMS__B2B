...
const AdminUpdateApprovals = () => {
  ...
  const fetchCompletedRequests = async () => {
    try {
      const response = await axios.get(
        'https://employee-management-system-1-qs2v.onrender.com//api/admin-updates/completed-requests'
      );
  ...
  const handleApprove = async (requestId) => {
    setLoading(true);
    try {
      await axios.post('https://employee-management-system-1-qs2v.onrender.com//api/admin-updates/handle-request', {
        requestId,
        action: 'approve'
      });
